...
Phase 7C-R0 已完成三项独立诊断，允许进入 R1 开发，但仍不允许运行多温度 ANN。建议下一步按以下顺序工作：

1. 保持 Phase 7C 的48条轨迹为永久失败确认集，不用于热层调参；
2. 使用新的开发集实现60–300 s短时域热安全电流上限，因为5 s一步限制在20/24条原确认轨迹中出现热—斜率空区间；
3. 预注册一次固定替代起点重试：默认起点失败时只允许使用确定的替代起点重试一次；
4. 在新开发集上建立15/25/30 ℃分温度电压残差增长裕量；
5. 所有参数冻结后生成15/30 ℃各24个新独立初态，先验证安全 MPC；
6. 只有新安全 MPC严格通过，才运行240条冻结 ANN轨迹。

当前最需要保护的研究结论是：

> Level 3/3P 证明解析投影可以补上输入硬约束保证；Phase 7B-0/7B-1 证明在线电压残差安全层能修复25 ℃跨模型电压失配；Phase 7C 把新边界定位为热安全和温度相关模型失配；Phase 7C-R0 又证明30 ℃、35 ℃上限和80% SOC目标之间存在可行策略，最快DFN验证候选为1895 s、最高34.962 ℃。下一步应修复教师求解稳定性并增加短时域热安全监督层，而不是重训 ANN。

R0还证明：15 ℃失败状态在相同默认起点下20/20次复现状态码8，但返回计划严格可行；替代起点14/15次成功。30 ℃反转来自默认起点进入次优分支，其第一动作3.420 A，而替代分支为5.389 A，目标函数改善约35%。因此两项都属于教师求解稳定性问题，不能改写Phase 7C原失败。

Phase 7B-1 的保守性也必须保留：安全层约介入 41% 的控制步，相对无安全层基线平均增加约 4.69% 的充电时间。后续可以在新的调参集上研究随 SOC 或残差状态变化的增长界，但不能复用本次独立确认集进行调参并再次宣称独立通过。
