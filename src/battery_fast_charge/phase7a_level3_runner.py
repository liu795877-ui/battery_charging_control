"""执行 Phase 7A Level 3 的硬斜率教师、审计、DNN 和同模型闭环。"""
...
from .phase7a_level3_config import Level3DomainConfig, Phase7ALevel3Config
...
def _validate_parameters(config: Phase7ALevel3Config, root: Path) -> None:
    ...
    for key, value in (("r0_ohm", config.model.r0_ohm), ("r1_ohm", config.model.r1_ohm),
        ...
        if not np.isclose(value, identified[key], rtol=0.0, atol=1e-15):
            raise RuntimeError(f"Level 3 参数 {key} 未保持项目辨识值。")


def design_initial_states(config: Phase7ALevel3Config, model: Level3Model, domain: Level3DomainConfig,
    ...
    while len(records) < domain.trajectory_count:
        ...
        previous_current = (
            config.constraint.previous_current_bounds_a[0]
            + np.ptp(config.constraint.previous_current_bounds_a) * _van_der_corput(candidate, 7)
        )
        candidate += 1
        state = Level3State(soc, v1, v2, previous_current)
        minimum_next_current = max(
            model.inherited.mpc.current_bounds_a[0],
            previous_current - config.constraint.maximum_current_step_a,
        )
        if model.terminal_voltage(state, minimum_next_current) > voltage_limit:
            continue
        feasibility = Level3MPC(model).solve(state)
        if not feasibility.optimizer_success or not feasibility.prediction_feasible:
            continue
        records.append({"trajectory_id": f"{prefix}_{len(records):03d}", "initial_soc": soc,
                        ...
                        "initial_previous_current_a": previous_current})
...
def _teacher_trajectory(row: pd.Series, steps: int, model: Level3Model) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    controller = Level3MPC(model)
    state = Level3State(float(row.initial_soc), float(row.initial_polarization_1_v),
                        float(row.initial_polarization_2_v), float(row.initial_previous_current_a))
    records = []; rejection = ""
    for step in range(steps):
        ...
        records.append({"trajectory_id": row.trajectory_id, "split": row.split, "step_index": step,
                        ...
                        "state_polarization_2_v": state.polarization_2_v,
                        ...
                        "minimum_slew_margin_a": result.minimum_slew_margin_a,
                        "maximum_prediction_current_step_a": result.maximum_current_step_a,
...
def _generate_domain(config: Phase7ALevel3Config, model: Level3Model, domain: Level3DomainConfig,
    ...
    for index, (_, initial) in enumerate(design.iterrows(), start=1):
        ...
        if index % 10 == 0:
            ...
            print(f"Level 3 {prefix} teacher {index}/{len(design)}", flush=True)
...
def _representative_states(dataset: pd.DataFrame, count: int) -> pd.DataFrame:
    ...
    selected.insert(0, "state_id", [f"level3_audit_{i:03d}" for i in range(count)])
    return selected


def _level3_warm_starts(
    controller: Level3MPC,
    state: Level3State,
    state_index: int,
    count: int,
    seed: int,
) -> list[tuple[str, np.ndarray | None]]:
    """将既有多起点合同投影为满足新增硬斜率约束的多样可行起点。"""
    lower, upper = controller.config.mpc.current_bounds_a
    limit = controller.model.config.constraint.maximum_current_step_a
    projected: list[tuple[str, np.ndarray | None]] = []
    for kind, values in _warm_starts(controller, state_index, count, seed):
        if values is None:
            projected.append((kind, None))
            continue
        feasible = np.empty_like(values, dtype=float)
        previous = state.previous_current_a
        for index, value in enumerate(values):
            feasible[index] = np.clip(value, previous - limit, previous + limit)
            feasible[index] = np.clip(feasible[index], lower, upper)
            previous = feasible[index]
        projected.append((kind, feasible))
    return projected


def _teacher_audit(config: Phase7ALevel3Config, model: Level3Model, dataset: pd.DataFrame,
    ...
    if solutions.empty:
        ...
        for state_index, row in selected.iterrows():
            state = Level3State(float(row.state_soc), float(row.state_polarization_1_v),
                                float(row.state_polarization_2_v), float(row.state_previous_current_a))
            controller = Level3MPC(model)
            starts = _level3_warm_starts(
                controller,
                state,
                state_index,
                config.data.warm_starts_per_state,
                config.data.random_seed,
            )
            for warm_index, (kind, warm) in enumerate(starts):
            ...
            if (state_index + 1) % 10 == 0:
                ...
                print(f"Level 3 audit {state_index+1}/{len(selected)}", flush=True)
...
def _train(config: Phase7ALevel3Config, model: Level3Model, dataset: pd.DataFrame, output: Path, resume: bool) -> tuple[pd.DataFrame, dict[int, TinyANN]]:
    ...
    for seed in model.inherited.network.initialization_seeds:
        model_path = model_dir / f"level3_deep_lbfgs_seed_{seed}.npz"
        ...
        record: dict[str, Any] = {"seed": seed, "architecture": "4-32-32-16-1", "optimizer": "lbfgs",
        ...
        print(f"Level 3 DNN seed {seed}: global={100*record['test_nrmse']:.3f}% terminal={100*record['terminal_test_nrmse']:.3f}%", flush=True)
    return pd.DataFrame(records.values()).sort_values("seed"), networks


def _closed_initial_states(config: Phase7ALevel3Config, model: Level3Model) -> pd.DataFrame:
    ...
    while len(records) < d.closed_loop_trajectory_count:
        ...
        v2 = d.closed_loop_v2_bounds_v[0] + np.ptp(d.closed_loop_v2_bounds_v)*_van_der_corput(candidate,5)
        previous_current = (
            config.constraint.previous_current_bounds_a[0]
            + np.ptp(config.constraint.previous_current_bounds_a) * _van_der_corput(candidate, 7)
        )
        ...
        if model.terminal_voltage(state,minimum_next_current) > model.inherited.mpc.terminal_voltage_max_v-config.data.initial_voltage_margin_v:
            continue
        records.append({"trajectory_id":f"level3_closed_{len(records):02d}","initial_soc":soc,
                        "initial_polarization_1_v":v1,"initial_polarization_2_v":v2,
                        "initial_previous_current_a":previous_current})
    return pd.DataFrame(records)


def _rollout(config: Phase7ALevel3Config, model: Level3Model, controller: Level3MPC | TinyANN,
             initial: pd.Series, kind: str, seed: int | None=None) -> list[dict[str,Any]]:
    state = Level3State(float(initial.initial_soc),float(initial.initial_polarization_1_v),
                        float(initial.initial_polarization_2_v),float(initial.initial_previous_current_a)); rows=[]
    for step in range(config.data.maximum_closed_loop_steps):
        ...
        if kind=="mpc":
            ...
            current=float(controller.predict(np.asarray([
                state.soc,state.polarization_1_v,state.polarization_2_v,state.previous_current_a
            ])))  # type: ignore[union-attr]
            elapsed=perf_counter()-started
        voltage=model.terminal_voltage(state,current); current_step=abs(current-state.previous_current_a); next_state=model.step(state,current)
        rows.append({"controller":kind,"seed":seed,"trajectory_id":initial.trajectory_id,"step_index":step,
                     ...
                     "previous_current_a":state.previous_current_a,"current_a":current,
                     "current_step_a":current_step,"terminal_voltage_v":voltage,"next_soc":next_state.soc,"elapsed_s":elapsed})
        state=next_state
        if state.soc>=model.inherited.mpc.target_soc-5e-4: break
    return rows


def _closed_loop(config: Phase7ALevel3Config, model: Level3Model, networks: dict[int,TinyANN], data_dir:Path, resume:bool) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame]:
    ...
    for index,(_,state) in enumerate(initial.iterrows(),start=1):
        rows.extend(_rollout(config,model,Level3MPC(model),state,"mpc")); print(f"Level 3 closed-loop MPC {index}/{len(initial)}",flush=True)
    ...
    for seed in networks:
        ...
        slew_violation=np.maximum(dnn.current_step_a.to_numpy()-config.constraint.maximum_current_step_a,0.0)
...
def _plots(output:Path,dataset:pd.DataFrame,audit:pd.DataFrame,offline:pd.DataFrame,closed:pd.DataFrame)->None:
    ...
    axes[0].set(xlabel="SOC",ylabel="V1+V2 [V]",title="2RC + slew teacher coverage"); fig.colorbar(scatter,ax=axes[0],label="Current [A]")
    ...
    fig,axes=plt.subplots(1,3,figsize=(15,4.2)); axes[0].bar(offline.seed.astype(str),100*offline.test_nrmse,label="Global"); axes[0].bar(offline.seed.astype(str),100*offline.terminal_test_nrmse,alpha=.6,label="Terminal"); axes[0].axhline(1,color="red",linestyle="--"); axes[0].legend(); axes[0].set(ylabel="Frozen-test NRMSE [%]")
    ...
    axes[2].bar(closed.seed.astype(str),closed.maximum_current_step_a); axes[2].axhline(2,color="red",linestyle="--"); axes[2].set(ylabel="Maximum current step [A]",title="Hard slew gate")
...
def _write_report(path:Path,p:dict[str,Any])->None:
    t,a,o,c,d=p["teacher"],p["teacher_audit"],p.get("offline"),p.get("closed_loop"),p["decision"]
    offline_text = (
        f"""- 全域冻结测试 NRMSE：{100*o['global_test_nrmse_min']:.4f}%–{100*o['global_test_nrmse_max']:.4f}%。
...
- 最大闭环单步电流变化：{c['maximum_current_step_a']:.6f} A；最大斜率违约：{c['maximum_slew_violation_a']:.6f} A。"""
        if o is not None and c is not None
        else "- 教师确定性审计未通过；依据预注册停止条件，本级未训练 pure DNN，也未运行 DNN 闭环。"
    )
    path.write_text(f"""# Phase 7A Level 3：硬斜率约束 pure DNN 验证报告

## 结论

Level 3 判定：**{d['conclusion']}**。允许进入 Level 4：**{'是' if d['proceed_to_level4'] else '否'}**。
...
def run_phase7a_level3(config:Phase7ALevel3Config,project_root:str|Path,resume:bool=False)->dict[str,Any]:
    root=Path(project_root).resolve(); data_dir=root/"data"/"phase7a_level3_slew"; output=root/"outputs"/"phase7a_level3_slew"; data_dir.mkdir(parents=True,exist_ok=True); output.mkdir(parents=True,exist_ok=True)
    _validate_parameters(config,root)
    ...
    model=Level3Model(config,inherited,root)
    ...
    frozen_hashes={}
    for split, filename in (("test","global_frozen_test.csv"),("terminal_test","terminal_frozen_test.csv")):
        frozen=dataset[dataset.split==split].copy()
        frozen.to_csv(data_dir/filename,index=False)
        frozen_hashes[split]={"path":filename,"sha256":_sha256(data_dir/filename),
                              "trajectory_count":int(frozen.trajectory_id.nunique()),"sample_count":len(frozen)}
    (data_dir/"frozen_test_hashes.json").write_text(json.dumps(frozen_hashes,ensure_ascii=False,indent=2),encoding="utf-8")
    ...
    teacher_checks={"acceptance":bool(acceptance>=inherited.gates.minimum_teacher_acceptance_fraction),"complete_accepted_trajectories":bool(all(actual_steps[k]==v for k,v in expected_steps.items())),
                    ...
                    "hard_slew_constraint":bool((dataset.minimum_slew_margin_a>=-inherited.mpc.constraint_tolerance).all()),
                    "complete_frozen_test_domains":bool(
                        frozen_hashes["test"]["trajectory_count"]==config.data.global_domain.test_trajectory_count
                        and frozen_hashes["terminal_test"]["trajectory_count"]==config.data.terminal_domain.test_trajectory_count
                    )}
    ...
    _,audit_table,audit_metrics=_teacher_audit(config,model,dataset,data_dir,resume); payload={"study_name":config.study_name,"configuration":asdict(config),"inherited_level2_configuration":asdict(level2),"inherited_level1_configuration":asdict(inherited),"frozen_test_hashes":frozen_hashes,"teacher":teacher,"teacher_audit":audit_metrics}
    ...
    if success:
        conclusion="Level 3 通过"
    elif not teacher_gate:
        conclusion="Level 3 教师审计未通过，停止 DNN 训练"
    elif not payload.get("offline",{}).get("success",False):
        conclusion="Level 3 离线 pure DNN 未通过，停止增加复杂度"
    elif not payload.get("closed_loop",{}).get("checks",{}).get("zero_slew_violation",True):
        conclusion="Level 3 硬斜率约束首次导致 pure DNN 闭环失败"
    else:
        conclusion="Level 3 同模型闭环未通过，停止增加复杂度"
    payload["decision"]={"checks":checks,"level3_success":success,"proceed_to_level4":success,"conclusion":conclusion}; payload["status"]="completed"; payload["success"]=success
    (output/"metrics.json").write_text(json.dumps(payload,ensure_ascii=False,indent=2,default=_json_default),encoding="utf-8"); _write_report(output/"PHASE7A_LEVEL3_中文实验报告.md",payload); print(json.dumps(payload["decision"],ensure_ascii=False,default=_json_default),flush=True); return payload
